...
class InterviewFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var interviewTopics: List<InterviewTopics> = ArrayList<InterviewTopics>()
        var topicTextView: TextView = view.findViewById(R.id.tvTopicText)

        fab.setOnClickListener {
            interviewViewModel.allTopics.observe(viewLifecycleOwner, Observer {
                it.let {
                    topicTextView.text = interviewTopics[0].toString()
                }
            })
        }
    }
